...

  {
    ...
    init = function()
      ...
      pcall(function()
        ...
        if vim.fn.isdirectory(shim_dir) == 1
          and not (':' .. vim.env.PATH .. ':'):find(':' .. vim.pesc(shim_dir) .. ':', 1, false) then
      ...
      vim.api.nvim_create_autocmd('FileType', {
        pattern = 'dbui',
        callback = function()
          if autoreload_started then return end
          autoreload_started = true
          local ok, mod = pcall(require, 'features.dbui_autoreload')
          if ok then mod.start() end
        end,
      })
      vim.api.nvim_create_user_command('DBUIAutoReloadStatus', function()
        local ok, mod = pcall(require, 'features.dbui_autoreload')
        if not ok then return vim.notify('autoreload module not loaded', vim.log.levels.WARN) end
        local s = mod.status()
      ...
      vim.api.nvim_create_user_command('DBUIAbortLogin', function()
        pcall(function() require('features.dbui_sso').abort() end)
      end, { desc = 'Reset stuck SSO-login state' })

      vim.keymap.set('n', '<leader>dL', '<cmd>DBUILogin<cr>',
        { desc = 'DB: AWS SSO login + token refresh' })
      ...
      vim.schedule(function()
        local ok, mod = pcall(require, 'features.dbui_sso')
        if ok then mod.start_watcher() end
      end)
