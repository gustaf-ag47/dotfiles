...
local function resolve_connections_path()
  if config.connections_path then return config.connections_path end
  local saveloc = vim.g.db_ui_save_location
  if saveloc and saveloc ~= '' then return saveloc .. '/connections.json' end
...
local function check_sso()
  ...
  local out = vim.fn.system({ 'aws', 'sts', 'get-caller-identity',
  ...
  if expires_iso then
    ...
    local y, mo, d, h, mi, s = expires_iso:match(
      '^(%d+)-(%d+)-(%d+)T(%d+):(%d+):(%d+)Z?$')
    if y then
      expires_s = os.time({ year=y, month=mo, day=d, hour=h, min=mi, sec=s,
                            isdst=false })
      ...
      elseif secs_left < 900 then  -- < 15 min
        return {
          ok = true, warn = true,
          detail = ('SSO valid, expires in %dm%ds (%s)')
            :format(math.floor(secs_left/60), secs_left%60, expires_iso),
          hint = ':DBUILogin soon to avoid mid-query interruption',
        }
      else
        return {
          ok = true,
          detail = ('SSO valid, expires in %dh%dm (%s)')
            :format(math.floor(secs_left/3600), math.floor((secs_left%3600)/60), expires_iso),
        }
      end
    end
  end
...
local function check_tunnel()
  ...
  if not handle then return { ok = false, detail = 'uv.new_tcp() failed' } end
  local result = { ok = false, detail = 'no response' }
  local done = false
  handle:connect('127.0.0.1', config.tunnel_port, function(err)
    if err == nil then result = { ok = true } end
    handle:close()
    done = true
  end)
  -- Pump the event loop briefly
  vim.wait(800, function() return done end, 50)
  if result.ok then
...
local function check_token()
  ...
  if ok_proj then
    ...
    if s and s.shared_path then table.insert(paths, s.shared_path) end
  end
  ...
  for _, path in ipairs(paths) do
    if vim.fn.filereadable(path) == 1 then
      ...
      if ok_json and type(parsed) == 'table' then
        for _, c in ipairs(parsed) do table.insert(conns_all, c) end
      end
    end
  end
  if #conns_all == 0 then
    return { ok = false, detail = 'no connection files readable',
             hint = 'check ' .. table.concat(paths, ', ') }
  end
  ...
  for _, c in ipairs(conns_all) do
    if c.url then
      ...
      if d then
        local y, mo, da, h, mi, s =
          d:match('^(%d+)(%d%d)(%d%d)T(%d%d)(%d%d)(%d%d)Z$')
        if y then
          local t = os.time({ year=y, month=mo, day=da,
                              hour=h, min=mi, sec=s, isdst=false })
          local age = now_utc - t
  ...
  if newest_age_s > 900 then
    return {
      ok = false,
      detail = ('newest IAM token is %dm old (%s); X-Amz-Expires is 900s'):format(
        math.floor(newest_age_s/60), newest_date),
  ...
  elseif newest_age_s > 720 then  -- > 12 min, getting stale
    return {
      ok = true, warn = true,
      detail = ('newest IAM token is %dm%ds old (%s); rotation due'):format(
        math.floor(newest_age_s/60), newest_age_s%60, newest_date),
    }
  end
  return {
    ok = true,
    detail = ('newest IAM token is %dm%ds old (%s)'):format(
      math.floor(newest_age_s/60), newest_age_s%60, newest_date),
  }
end
...
local function check_autoreloader()
  ...
  if not ok then
    return { ok = false, detail = 'features.dbui_autoreload not loaded',
             hint = 'open DBUI once (triggers FileType=dbui autocmd that loads it)' }
  end
  local s = mod.status()
  if not s.active then
    return { ok = false, detail = ('watcher inactive (path=%s)'):format(tostring(s.path)),
...
local function check_cmp()
  ...
  if not ok then return { ok = false, detail = 'nvim-cmp not loaded' } end
  ...
  for _, src in ipairs(sources or {}) do
    if type(src) == 'table' and src.name then table.insert(names, src.name)
...
local function check_scope()
  ...
  if s.shared_path then detail = detail .. ', inherits=' .. s.shared_path end
  detail = detail .. ')'
